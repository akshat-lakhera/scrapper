...
export const SearchHistory: React.FC<SearchHistoryProps> = ({ setActiveTab }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Search Input Box */}
      <div className="rounded-2xl glow-hover stagger-in p-6 space-y-5" style={{ ...stagger(1), background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)' }}>
        <form onSubmit={handleSearch} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-3.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 btn-spring transition-all ${loading ? 'shimmer-loading' : ''}`}
            style={{
              ...
              {currentSearch.results.map((res: any, idx: number) => (
                <div key={idx} className="p-5 rounded-2xl glow-hover card-reveal space-y-3" style={{ ...stagger(idx), background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)' }}>
                  ...
                  <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                    {res.seller || res.company} {res.location ? `• ${res.location}` : ''}
                  </div>
                  {(res.price || res.salary) && (
